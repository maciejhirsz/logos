...
impl<'source> Logos<'source> for Tokens {
    type Extras = bool;
    type Source = str;
    type Error = ();
    fn lex(lexer: &mut Lexer<'source, Self>) -> Option<Result<Self, Self::Error>> {
        if lexer.extras {
            ...
            result.map(|outer| outer.map(Tokens::OuterToken))
        }
    }
}

#[test]
fn lex_morphed_tokens_in_different_modes() {
    use Inner::*;
    use Tokens::*;
    let s = r#""Hello W\u{00f4}\162ld\n""#;

    let results: Vec<Result<Tokens, ()>> = Tokens::lexer(s).collect();
    let expect = vec![
        Ok(OuterToken(Outer::StartString)),
        Ok(InnerToken(Text)),
        Ok(InnerToken(EscapedCodepoint)),

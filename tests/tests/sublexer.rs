...
#[logos(extras_lifetime = 'extras, extras = &'extras mut bool)]
enum Alpha<'s> {
    #[regex("[a-z]+")]
    Value(&'s str),
    #[token("|")]
...
impl<'s> Logos<'s> for AlphaNumeric<'s> {
    ...
    fn lex<'b>(lexer: &mut Lexer<'s, '_, Self>) -> Option<Result<Self, Self::Error>> {
        if !lexer.extras {
            let result = {
                let mut sublexer = lexer.sublexer_with::<Alpha>(|x| x);
                sublexer.next()?
            };
            ...
            Some(Ok(AlphaNumeric::Alpha(that)))
        } else {
            let result = {
            ...
            Some(Ok(AlphaNumeric::Numeric(that)))
        }
    }
}

...
impl<'s> ::logos::Logos<'s> for Token {
    ...
    fn lex(lex: &mut ::logos::Lexer<'s, Self>) {
        ...
        fn _end<'s, 'e>(lex: &mut Lexer<'s, 'e>) {
            lex.end()
        }
        fn _error<'s, 'e>(lex: &mut Lexer<'s, 'e>) {
        ...
        #[inline]
        fn goto1_x<'s, 'e>(lex: &mut Lexer<'s, 'e>) {
            lex.set(Ok(Token::Letter));
        }
        #[inline]
        fn goto3_at1_with3<'s, 'e>(lex: &mut Lexer<'s, 'e>) {
        ...
        #[inline]
        fn goto4<'s, 'e>(lex: &mut Lexer<'s, 'e>) {

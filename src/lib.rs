...
pub trait Logos<'source>: Sized {
    ...
    fn lex(lexer: &mut Lexer<'source, '_, Self>) -> Option<Result<Self, Self::Error>>;

    /// Create a new instance of a `Lexer` that will produce tokens implementing
    /// this `Logos`.
    fn lexer<'extras>(source: &'source Self::Source) -> Lexer<'source, 'extras, Self>
    where
    ...
    fn lexer_with_extras<'extras>(
        source: &'source Self::Source,
        extras: Self::Extras<'extras>,
    ) -> Lexer<'source, 'extras, Self> {
...
/// ```
#[inline]
pub fn skip<'source, 'extras, Token: Logos<'source>>(
    _: &mut Lexer<'source, 'extras, Token>,
) -> Skip {
    Skip
}

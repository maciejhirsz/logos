...
mod tests {
    ...
    fn insert_a_fork_into_a_fork() {
        ...
        let mut parent = Node::Fork(Fork::new(Plain).arm(Branch::new("abc").then(&token_a)));

        parent.insert(Fork::new(Plain).arm(Branch::new("def").then(&token_b)));
    ...
    fn collapsing_a_fork() {
        ...
        let mut fork = Fork::new(Maybe).arm(Branch::new("abc")).then(&token_a);

        let expected = Fork::new(Maybe)
            .arm(Branch::new("abc").then(&token_a))
            .then(&token_a);
    ...
    fn unwinding_a_fork() {
        ...
        let mut fork = Fork::new(Repeat).arm(Branch::new("abc")).then(&token_a);
    ...
    fn insert_a_repeat_fork_into_a_fork() {
        ...
        parent.insert(Fork::new(Repeat).arm(Branch::new("def")).then(&token_b));
        ...
        let expected = Node::Fork(
            Fork::new(Maybe)
                ...
                .arm(
                    Branch::new("def")
                        .then(Fork::new(Repeat).arm(Branch::new("def")).then(&token_b)),
                )
                .then(&token_b),
        );
    ...
    fn insert_a_fork_into_a_repeat_fork() {
        ...
        let mut parent = Node::Fork(Fork::new(Repeat).arm(Branch::new("def")).then(&token_b));

        parent.insert(Fork::new(Plain).arm(Branch::new("abc").then(&token_a)));
    ...
    fn pack_analog_branches() {
        ...
        let expected = Node::Branch(Branch::new(Pattern::Range(b'a', b'c')));
    ...
    fn pack_analog_branches_with_leaves() {
        ...
        let expected = Node::Branch(Branch::new(Pattern::Range(b'a', b'c')).then(&token));
    ...
    fn pack_branches() {
        let mut node =
            Node::Branch(Branch::new("abc").then(Branch::new("def").then(Branch::new("xyz"))));

        node.pack();

        let expected = Node::Branch(Branch::new("abcdefxyz"));
        ...
        let expected = Fork::new(Plain)
            .arm(
                Branch::new("a")
                    ...
                    .fallback(
                        Fork::new(Repeat)
                            .arm(Branch::new(Pattern::new("ab".bytes())))
                            .then(&ab),
                    ),
            )
            .arm(
